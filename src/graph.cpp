...
// void BiGraph::TrimButterfly(int p, int q,int dep = 0) {
//     std::vector<std::vector<std::pair<int,int>>> left_neighbour_edge(left_n_);
//     std::vector<std::vector<std::pair<int,int>>> right_neighbour_edge(right_n_);
//     {
//         int cnt = 0;
//         for (auto [u, v] : edge_) {
//             left_neighbour_edge[u].emplace_back(v,cnt);
//             cnt++;
//         }
//     }
//     for (int u = left_n_; u --> 0; ) {
//         for (auto [v,edge_id] : left_neighbour_edge[u]) {
//             right_neighbour_edge[v].emplace_back(u,edge_id);
//         }
//     }
//     std::fill(left_neighbour_edge.begin(), left_neighbour_edge.end(), std::vector<std::pair<int,int>>());
//     for (int v = right_n_; v --> 0; ) {
//         for (auto [u,edgeid] : right_neighbour_edge[v]) {
//             left_neighbour_edge[u].emplace_back(v,edgeid);
//         }
//     }

//     std::vector<int> butterfly_cnt(edge_n_);
//     std::vector<int> left_buffer(left_n_);
//     for (int i = 0; i < left_n_; i++) {
//         for (auto [j,eid1] : left_neighbour_edge[i]) {
//             for (auto [k,eid2] : right_neighbour_edge[j]) {
//                 if (k <= i) {
//                     break;
//                 }
//                 left_buffer[k]++;
...
//                     break;
//                 }
//                 butterfly_cnt[eid1] += left_buffer[k] - 1;
//                 butterfly_cnt[eid2] += left_buffer[k] - 1;
//             }
//         }
//         for (auto [j,eid1] : left_neighbour_edge[i]) {
//             for (auto [k,eid2] : right_neighbour_edge[j]) {
...
//         }
//     }
//     int original_edge_count = edge_.size();
//     std::vector<std::pair<int, int>> new_edge;
//     for (int i = 0; i < edge_n_; i++){
//         //std::cout << butterfly_cnt[i] << "\n";
//         if (butterfly_cnt[i]>=(p-1)*(q-1)) new_edge.emplace_back(edge_[i]);
//     }
//     LoadFromEdges(std::move(new_edge));
//     Trim(p,q);

//     int now_edge_count = edge_.size();
//     if (dep < 0 && original_edge_count > now_edge_count){
//         TrimButterfly(p, q, dep+1);
//     }
// }
